...
8. **同じ音声を2回流す。** 候補自身の出力が実行ごとに変わるかを見る（現行は固定wavの再実行で25/25一致。録音時とwav再投入の差とは区別する。S3 §6.1）。
...
現行では発話間や録音時とwav再投入の間で表記が揺れる（`I think` が8通り、`iphene` / `iphone`）。ただし同じ固定wavの再実行は25/25一致している。**候補ではどうか。**
