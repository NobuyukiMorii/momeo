...
| 通知許可ゲートの適用範囲 | **決定**。ゲートは Android だけ（規約由来）。iOS は通知を使わず、許可も求めない（録音中の表示は Live Activity で出す。通知の許可は不要 → `impl/phase2/step06_recording_indicator_on_ios.md`）。根拠: 録音中表示の不足を咎めた審査事例がオレンジの点の導入（2020年9月）以降見つからず、点だけに頼る録音アプリも多数ある。Apple DTS もオレンジの点を録音中の表示と認めている（https://developer.apple.com/forums/thread/776949） |
| 許可を拒否されたときの表示 | **決定**。OS の通知許可ダイアログは使わず、開示ダイアログが許可への導線を兼ねる。許可が無い間は「有効にする」が端末の設定を開き、許可して戻るとそのまま ON になって完了する。許可せずに戻ればダイアログが開いたまま残るだけで、エラーダイアログは無い。ON の後に OS 設定で取り消されたケースは画面に何も出さず、次に ON にする操作で開示ダイアログがもう一度出て回収する |
