...
計画は成立する。両OSとも「OFF は背面で止まり、ON は背面でも録り続ける」ところまで動いた。
...
|---|---|---|
| OFF: 背面で停止 → 前面で再開（Step 1） | ✅ 復帰後の発話も転写された | ✅ ログと appops で確認 |
| ON への切り替えの流れ（Step 3） | ✅ 開示 → OS 許可ダイアログ → 保存 | ✅（設定の直書きで代替） |
| ON: 背面でも録音が続く（Step 2・5） | ✅ 背面中の発話がメモに転写された | ✅ 背面でもマイク使用が継続 |
| microphone 型のサービス起動（Step 2） | — | ✅ `types=0x00000080` |
| 常駐通知が通常欄に出る（Step 4） | — | ✅ サイレント欄ではなく通常欄 |
| 通知の停止ボタン（Step 4） | — | ✅ 録音停止＋設定 OFF＋サービス停止 |
| 背面中の「録音中」ローカル通知（Step 6） | ✅ 背面で表示・前面復帰で消える | — |

iOS の背面録音はデバッガ非接続のシミュレーターで動いた。強い材料だが、判定は release 実機で行う（→ `ios.md`）。
...
### 3. iOS は背面遷移の瞬間だとまだ「前面」扱いで、通知が表示されない（Step 6）

- paused を受けてすぐ出したローカル通知は、前面アプリ向けの表示判定（willPresent）に回されて**表示されずに終わる**。通知の登録自体は成功するため、エラーも出ない
- 対処: 1秒待ってから出す。待つ間に前面へ戻っていたら出さない（`lib/stt/listening_background_notice.dart`）
...
| 論点 | 状態 |
|---|---|
| 停止ボタンの効き方 | **決定**。設定ごと OFF に切り替える（通知からの停止 = 同意の撤回。再開はアプリを開けば前面録音として自動で始まる）。文言はスパイクの「録音を停止」から「**バックグラウンド録音を停止**」に変える（何が止まるかを正確に言う） |
| 前面表示中に停止ボタンを押されたとき | **本実装で直す**。スパイク実装は前面でも録音を止めてしまい、「このアプリを使っている時だけ録音」の表示と矛盾する。OFF に切り替えつつ録音は続ける形にする |
...
- iOS: release ビルド・実機での背面録音（ON と同じ重みで OFF 側も）
