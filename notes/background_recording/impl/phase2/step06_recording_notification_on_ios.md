...
- 通知の許可は、設定を ON に切り替える流れの中で OS のダイアログで求める（Step 3 の Android と違い、端末の設定への誘導はしない）。そして Android と違い、**拒否されても設定を ON にする**。許可を条件にしない（審査事例と録音アプリの調査から、オレンジの点だけでも先例が多数あると確認済み → `../../spike_findings.md` の決定の表）
