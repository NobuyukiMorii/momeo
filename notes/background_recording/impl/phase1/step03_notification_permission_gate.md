...
flowchart LR
    ...
    subgraph ng["拒否されたとき"]
        b1["「有効にする」を押す"] --> b2["OS が通知の許可を尋ねる"] --> b3[拒否] --> b4["設定は OFF のまま<br/>今までどおり"]
    end
```
...
「有効にする」が押されたら `POST_NOTIFICATIONS` を要求し、取れたときだけ `backgroundRecordingProvider` へ保存する。

- 許可は OS 設定から後で取り消せる。だから設定が ON のまま録音を始める場面でも毎回確認し、無ければサービスを立てず Step 1 の挙動（バックグラウンドで止まる）に落とす。このときのユーザーへの見せ方は着手時に決める（未設計）
