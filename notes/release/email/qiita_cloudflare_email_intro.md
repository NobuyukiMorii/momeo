...
レコードの追加はどうかというと、**メールアドレスを作るだけなら、ここで追加するレコードはありません**。

メールに必要なMXレコードとSPFレコードは、あとのステップでEmail Routingが自動で追加してくれます。
なので、ゾーンは空っぽのままステップ2に進んでOKです。

> 僕は同じドメインで、プライバシーポリシーページをGitHub Pagesで公開もしています。そのためのDNSレコードはここで一緒に追加できますが、メールとは独立した話なので、別記事にまとめました。（TODO: 公開後にサイト公開記事のリンクを貼る）
