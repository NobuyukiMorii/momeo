...
// ============================================================
// 文字化の動作確認セクション
//   マイク → 音声 isolate（VAD の区切り → 認識モデル）→ 認識テキストの表示。
...
class _SttTranscriptionSectionState extends State<SttTranscriptionSection> {
  ...
  Future<void> _prepare() async {
    try {
      ...
      // 認識モデル（本体・tokens）が無いと文字化できない
      if (!models.sttModel.isValid || !models.sttTokens.isValid) {
        throw StateError(
          '認識モデルが未配置です。「STT → モデル配置」が OK か確認してください',
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        Text(
          'マイクの音を VAD で発話ごとに区切り、認識モデルでテキストに変換します。\n'
