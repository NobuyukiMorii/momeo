...
ここが Step 5 の肝。後の Step 9 では「**1つの発話チャンク = メモカード1枚**」としてつなぐ。
...
この箱は最終的に本番（Step 9 のリスニング画面）でも使うが、**まずは検証セクション内に書いて動かす**。本番で必要になった時（Step 9）に小さなクラスへ切り出す。最初から共通化を作り込まない（可読性優先）。
...
- 大きい NeMo モデルの**配布** → Step 6
- リスニング画面への**本配線**（チャンク → カード） → Step 9
