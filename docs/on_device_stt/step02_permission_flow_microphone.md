...
- **ドキュメントの扱い（決定）**: 音声認識「権限」に関する記述は、混乱を残さないよう **本ステップで整理する**（§3）。`outline.md` Step 8（仕様ドキュメントの改訂）は予定どおり残し、`listening_flow.md` / `overview.md` の改訂は Step 8 で行う。
...
| `docs/specs/listening_flow.md` / `docs/specs/overview.md` | 確定トリガー・全体フローの記述 | **Step 8** で改訂 |
