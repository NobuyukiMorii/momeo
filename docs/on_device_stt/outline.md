...
- **目的**: 準備が失敗したら**ボタンなしで自動再試行**し続け、ユーザー操作ゼロで復帰させる（準備が終わらない限りアプリは使えず、失敗時にユーザーが取れる行動は再試行しかないため、押させる意味がない）。
- **やること**:
  - **再試行の入口（エンジン準備のやり直し）を Step 9 の provider に足す**（再試行を呼ぶ主体はゲート自身。ユーザー操作は挟まない）。
  - DL起因の失敗（未開始・DL失敗）は、取得開始・再取得（Step 8）→ エンジン準備やり直し、の順に復帰する。
  - バックオフ付き（失敗のたびに間隔を広げる）で自動リトライする。表示は "Retrying"（Step 12 の仕組みに乗るだけ）。
- **完了の目安**: DLを失敗させても、通信回復後に操作なしでリスニングまで復帰する。失敗が続いても画面が固まったように見えない。
