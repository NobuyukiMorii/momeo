...
- **目的**: Step 7 で動いた「エンジンをメモリに読み込む」処理を**起動と同時にバックグラウンドで開始**し、エンジンをアプリ全体で1つだけ保持する。**このステップは待ち画面やブロックを持たない**（待たせる処理は Step 11 に集約する）。
- **やること**:
  - アプリ起動（スプラッシュ開始）と同時に、裏で準備を始める：メモリ展開（毎回）と、必要なら初回DL完了待ち（Android・Step 8 の `ensureReady`）。
  - 準備は特定の画面の持ち物にせず、**アプリ全体で1つだけ持つ provider**（シングルトン）が所有し、スプラッシュ → オンボーディング → … と画面が変わっても裏で走り続けるようにする。
  - 準備の**状態（準備中／完了／失敗）**を、後段（Step 11 の待ち画面）が参照できる形で公開する。
  - 読み込んだエンジンはアプリ全体で1つだけ保持して使い回す（`databaseProvider` と同じ形）。
- **完了の目安**: 起動と同時に準備が始まり、エンジンが1個だけ保持される。準備の状態を後段から参照できる。
- **備考**: 「準備できるまで待つ／待ち画面を出す」のは **Step 11** の担当。本ステップは UI ブロックを持たず、裏で準備を進めて状態を公開するところまで。
...
- **目的**: リスニングに入る直前で「準備が完了しているか」を判定し、未完了なら待ち画面を出して完了後にリスニングへ通す。**待たせる処理を1か所に集約する**。
- **やること**:
  - スプラッシュ → オンボーディング → … の後、リスニングに入る**直前の1か所**で、Step 9／Step 8 の準備状態を見てゲートする。
  - 未完了なら準備画面を表示する。状態（フェーズ）に応じて文言を出し分ける：Android 初回のDL中は進捗、メモリ展開中は「準備中」、失敗時はエラーと再試行。
  - 準備済みなら待ち画面を出さずに素通りでリスニングへ進む。
  - これにより、スプラッシュ・オンボーディングでは足止めせず、その時間を裏の準備に充てられる。
- **完了の目安**: モデル未準備の初回でも、待ち画面を経て自然にリスニングへ繋がる。準備済みなら待ち画面が出ずに素通りする。画面が固まったように見えない。
...
> - ① 起動時の生成（数秒）で、スプラッシュ／オンボーディングが目に見えてカクつくか。
> - ② 転写中（100〜300ms）に喋った音声を取りこぼすか（長い発話・連続発話）。
>
