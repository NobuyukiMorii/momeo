...
# ネットワーク表示はまとめて1回で設定する（分けて投げるとアイコンが重複する）。
# mobile hide と nosim hide は、SIM の無いエミュレータで ×印のアイコンが並ぶのを防ぐ
adb -s "$SERIAL" shell am broadcast -a com.android.systemui.demo -e command network -e wifi show -e level 4 -e fully true -e mobile hide -e nosim hide
