...
struct whisper_params
{
    ...
    // encoder の枠（既定 1500=30秒）を上書きする。0 で既定。
    // VAD の発話は数秒なので、小さくすると encoder の計算量が減り高速化する。
    int32_t audio_ctx = 0;
...
// -----------------------------------------------------------------
// モデル常駐（毎回 init/free しない）
//
// 同じモデルパスなら whisper_context を使い回す。発話ごとに数百MBの
// モデルをロードし直すコストを無くすのが狙い。モデルパスが変わったら作り直す。
// 転写は Dart 側で直列化している前提（同時に1件だけ呼ばれる）。
// -----------------------------------------------------------------
static struct whisper_context *g_ctx = nullptr;
static std::string g_model_path = "";

static struct whisper_context *get_or_init_context(const std::string &model_path)
{
    // 既に同じモデルを読み込んでいれば使い回す
    if (g_ctx != nullptr && g_model_path == model_path)
    {
        fprintf(stderr, "[whisper_momeo] reuse resident context (%s)\n", model_path.c_str());
        return g_ctx;
    }
    // モデルが変わった場合は古いものを解放してから読み直す
    if (g_ctx != nullptr)
    {
        whisper_free(g_ctx);
        g_ctx = nullptr;
    ...
    fprintf(stderr, "[whisper_momeo] loading model into resident context (%s)\n", model_path.c_str());
...
json transcribe(json jsonBody) noexcept
{
    ...
    // audio_ctx は後から追加したキー。古い呼び出しとの互換のため、無ければ既定(0)。
    if (jsonBody.contains("audio_ctx"))
    {
        params.audio_ctx = jsonBody["audio_ctx"];
    }
    json jsonResult;
    ...
    // whisper init（常駐コンテキストを取得。同じモデルなら使い回す）
    struct whisper_context *ctx = get_or_init_context(params.model);
    // モデルのロードに失敗したら、NULL を触って落ちる前にエラーを返す
    if (ctx == nullptr)
    {
        jsonResult["@type"] = "error";
    ...
    // run the inference
    {
        ...
        // encoder の枠を縮める（0 のときは whisper 既定の 1500=30秒）
