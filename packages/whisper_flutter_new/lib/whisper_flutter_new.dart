...
class Whisper {
  ...
  Future<Map<String, dynamic>> _request({
    ...
    // DTO が作る JSON に追加キー（audio_ctx 等）を差し込む。
    // freezed の DTO を再生成せずに済むよう、ここで文字列を組み立て直す。
    final Map<String, dynamic> requestMap =
        json.decode(whisperRequest.toRequestString()) as Map<String, dynamic>;
    requestMap.addAll(extra);
    final String requestString = json.encode(requestMap);

    return Isolate.run(
      () async {
        final Pointer<Utf8> data = requestString.toNativeUtf8();
  ...
  ///
  /// [audioCtx] は encoder の枠（既定 1500=30秒）を上書きする。0 なら既定。
  /// VAD の短い発話では小さくする（例: 256〜512）と転写が速くなる。
  Future<WhisperTranscribeResponse> transcribe({
    ...
    final Map<String, dynamic> result = await _request(
      ...
      extra: audioCtx > 0 ? {"audio_ctx": audioCtx} : const {},
    );
    if (kDebugMode) {
